...
def _pinyin_overlap(a: str, b: str, min_ratio=0.6) -> bool:
    """判断 a 与 b 的拼音是否足够重叠"""
    pa = _to_pinyin(a)
    pb = _to_pinyin(b)
    if not pa or not pb:
    ...
    overlap_ratio = max_len / min(len(pa), len(pb))
    return overlap_ratio >= min_ratio


# <<< PATCH >>> 字段滑窗生成器（不 normalize）
def _iter_name_segments(name: str, min_len: int = 4, max_len: int = 12):
    """
    从文件名中生成连续子片段（滑窗）
    不做 normalize，不假设命名规则
    """
    length = len(name)
...
class MusicLibrary:
    ...
    def find_real_music_name(self, name: str, n: int):
        ...
        def extract_song_name(full_title):
            return full_title.split(" - ", 1)[1] if " - " in full_title else full_title

        def extract_last_cn(song_name):
            segs = re.findall(r'[\u4e00-\u9fff]+', song_name)
            return segs[-1] if segs else ""

        def to_py(s):
            return "".join(lazy_pinyin(s))

        def is_pure_title(song_name: str) -> bool:
            """
            纯主标题定义：
            - 中文开头
            - 不包含明显“中文前缀修饰”
            ...
            # 必须是中文开头
            if not re.match(r'^[\u4e00-\u9fff]', song_name):
                return False

            # 如果这些符号出现在第一个中文之后，才算副标题
            for m in bad_marks:
                idx = song_name.find(m)
                if idx > 0:
                    return False
        ...
        query_is_cn = has_cn(query_raw)
        query_py = to_py(query_raw) if query_is_cn else query_norm.replace(" ", "")

        candidates = []

        for full_title in self.all_music.keys():
            song_name = extract_song_name(full_title)
            ...
            if query_is_cn:
                if len(query_norm) <= 3:
                    match_text = extract_last_cn(song_name)
                    pure = is_pure_title(song_name)

                    # ⭐ 如果最后中文段与查询完全一致（或拼音完全一致）
                    # 即使不是纯主标题，也允许通过
                    if not pure:
                        if not match_text:
                            continue

                        if normalize(match_text) != query_norm:
                            # 拼音也不完全一致才过滤
                            if to_py(match_text) != query_py:
                                continue
                else:
                    match_text = extract_last_cn(song_name)
            else:
                match_text = song_name

            match_norm = normalize(match_text)
            match_py = to_py(match_text) if query_is_cn else match_norm

            py_score = difflib.SequenceMatcher(None, query_py, match_py).ratio()
            char_score = difflib.SequenceMatcher(None, query_norm, match_norm).ratio()

            length_penalty = min(len(match_norm) / 20.0, 0.3) if len(query_norm) <= 4 else 0
            ...
            # ⭐ 副标题尾词轻微降权
            full_cn = "".join(re.findall(r'[\u4e00-\u9fff]+', song_name))
            if match_text and match_text != full_cn:
                score -= 0.05
            ...
            debug_info.update({
                "score_text": round(char_score, 3),
                "score_py": round(py_score, 3),
                "score_final": round(score, 3),
            })

        if not candidates:
            self.log.info(f"未命中歌曲：{name}")
            return []

        # 1️⃣ 完全等价主标题优先
        exact_main = []
        for c in candidates:
            song_name = extract_song_name(c["title"])
            full_cn = "".join(re.findall(r'[\u4e00-\u9fff]+', song_name))
            if full_cn == query_norm:
                exact_main.append(c)

        if exact_main:
            exact_main.sort(key=lambda x: x["score"], reverse=True)
            result = [c["title"] for c in exact_main[:n]]
        else:
            # 2️⃣ 再走原来的尾词等价逻辑
            exact = [
                c for c in candidates
                if c["match_norm"] == query_norm
            ]

            if exact:
                exact.sort(key=lambda x: x["score"], reverse=True)
                result = [c["title"] for c in exact[:n]]
            else:
                candidates.sort(key=lambda x: x["score"], reverse=True)
                result = [c["title"] for c in candidates[:n]]


        self.log.info(f"语音【{name}】→ 命中【{result}】")
        return result
